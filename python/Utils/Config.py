...
listConfigs=cf.read("sbs.conf")
if len(listConfigs)<=0:	listConfigs=cf.read("..\\src\\sbs.conf")
if len(listConfigs)>0:
    timeout_superbox_control=cf.getint("default", "timeout_superbox_control")
    ...
    maxcmd_superbox_control=cf.getint("default", "maxcmd_superbox_control")
...
dir_local_root=os.path.join(os.path.dirname(os.path.abspath(__file__)),os.path.pardir,os.path.pardir)
